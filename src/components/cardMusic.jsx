...
const CardMusic = ({imgUrl,nameSong,artistName,artistsLinkYoutube,id}) => {
    ...
    const [iconPlayer, setIconPlayer] = useState(false);

    return ( 
        <>
            <div className="flex flex-col min-w-[15%] w-[15%] relative mb-4">
                <div className="relative" >
                    <img src={imgUrl} alt="" className="rounded cursor-pointer" onClick={()=>setIconPlayer(!iconPlayer)} />
                    {iconPlayer ? <i className={`bi bi-pause-fill ${classNameRep}`} onClick={()=>setIconPlayer(!iconPlayer)}></i> : <i className={`bi bi-play-fill ${classNameRep}`} onClick={()=>setIconPlayer(!iconPlayer)}></i>}
                </div>
